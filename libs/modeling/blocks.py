...
class MaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        # Head_Channels
        #self.n_channels = n_embd // n_head
        self.n_channels = n_embd  #  此处每个head保持C通道数不变，不进行划分
        self.scale = 1.0 / math.sqrt(self.n_channels)
        ...
        # QKV  B,C,T=> 4个B,C,T  每个QKV输出通道数变成4C
        self.key = nn.Conv1d(self.n_embd, self.n_embd*self.n_head, 1)
        self.query = nn.Conv1d(self.n_embd, self.n_embd*self.n_head, 1)
        self.value = nn.Conv1d(self.n_embd, self.n_embd*self.n_head, 1)
        ...
        # 添加sk进行head 选择
        self.sk=ESK(self.n_embd,self.n_head)
